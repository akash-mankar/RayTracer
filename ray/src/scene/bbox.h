...
public:
	...
	BoundingBox* copy() { 
		...
		copy->bEmpty = this->bEmpty; 
		copy->dirty = this->dirty; 
		copy->bmin = this->bmin; 
		copy->bmax = this->bmax; 
		copy->bArea = this->bArea; 		
		copy->bVolume = this->bVolume;
	}
	...
	inline	bool intersects(const BoundingBox &target) const {
	...
	inline bool intersects(const Vec3d& point) const {
