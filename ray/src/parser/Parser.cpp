...
void printTree(KDTreeNode* node) { 
  if(node->isLeaf) { 
    ...
    cout<<"left "<<node->left->faces.size(); 
    cout<<"right "<<node->right->faces.size();  
    printTree(node->left); 
...
Material* Parser::parseMaterial( Scene* scene, const Material& parent )
{
  ...
  for( ;; )
  {
    ...
    switch( token->kind() )
    {
      ...
      case NORM:
      {
    	  MaterialParameter map = parseVec3dMaterialParameter(scene);
    	  mat->setMapValue( map);
    	  mat->setBumpMap( true);
    	  break;
      }
...
MaterialParameter Parser::parseVec3dMaterialParameter( Scene* scene )
{
  ...
  else if( _tokenizer.CondRead( MAP ) )
  {
    _tokenizer.Read( LPAREN );
    string filename = _basePath;
    filename.append( "/" );
