...
public:
	...
	TextureMap* getNormalTexture( string name );
	...
	void setTreeHeight(int height) {
		treeHeight = height;
		for(int j=0;j<objects.size();j++) {
			objects[j]->treeHeight = height;

		}
	}

	void setLeafSize(int height) {
			treeHeight = height;
			for(int j=0;j<objects.size();j++) {
				objects[j]->leafSize = height;

			}
	}

	void setTraceUI (TraceUI* traceUI) {
		this->traceUI = traceUI;
		for(int j=0;j<objects.size();j++) {
						objects[j]->traceUI = traceUI;
		}
...
private:
	...
	typedef std::map< std::string, TextureMap* > nmap;
	nmap nmapCache;
