...
Scene::~Scene() {
	...
	//for( n = nmapCache.begin(); n != nmapCache.end(); n++ ) delete (*n).second;
}
...
TextureMap* Scene::getNormalTexture( string name ) {
	tmap::const_iterator itr = nmapCache.find( name );
	if( itr == nmapCache.end() ) {
		nmapCache[ name ] = new TextureMap( name );
		return nmapCache[ name ];
	} else return (*itr).second;
}
