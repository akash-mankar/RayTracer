...
class TextureMap
{
    public:
       ...
       Vec3d getMappedValue( const Vec2d& coord ) const; // For texture Map
       Vec3d getNormalMappedValue( const Vec2d& coord ) const; // For normal Map
       void calcGradient(); // only for bump mapping
...
public:
    explicit MaterialParameter( const Vec3d& par )
      : _value( par ), _textureMap( 0 ), _normalMap(0)
    { }

    explicit MaterialParameter( const double par )
      : _value( par, par, par ), _textureMap( 0 ), _normalMap(0)
    { }

    explicit MaterialParameter( TextureMap* tex )
       : _textureMap( tex ), _normalMap(tex)
    { }

    MaterialParameter()
       : _value( 0.0, 0.0, 0.0 ), _textureMap( 0 ), _normalMap(0)
    { }
    ...
    Vec3d getNormalMapValue(const isect& is) const;
    void setNormalMapValue();
...
{
public:
	mutable std::vector <Photon> photons;
	bool bBumpMap;
   void addPhoton(Photon *photon) const{
	...
	virtual Vec3d shade( Scene *scene, const ray& r, isect& i ) const;
    ...
    // Normal Map  Material-> MaterialParameter->NormalMap routing
    Vec3d getMapValue( const isect& i ) const
    { return _map.getNormalMapValue(i);}
    ...
    void setMapValue(const MaterialParameter& map)
    {
    	_map = map;
    	_map.setNormalMapValue();
    }
    bool isBumpMap() const {return bBumpMap ;}
    void setBumpMap(bool b){ bBumpMap =b ;}
