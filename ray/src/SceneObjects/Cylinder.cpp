...
bool Cylinder::intersectLocal( const ray& r, isect& i ) const
{
	...
	if( intersectCaps( r, i ) ) {
		...
		bool intersect =  intersectBody( r, i );
		if(intersect == true)
			return intersect;
	}
}
...
bool Cylinder::intersectBody( const ray& r, isect& i ) const
{
	...
	if( t1 > RAY_EPSILON ) {
		...
		if( z >= 0.0 && z <= 1.0 ) {
			...
			if(z<0.0)
				z*=-1;
			//i.setUVCoordinates(Vec2d(0.5-(atan2(P[2],P[0])/(2*3.14)),y));*/
			/* Determine its angle from the x-z plane. */
			double phi = z; /* This will be from 0 to 1 */
			double theta = 0.0;
			z=y;
			...
			i.setUVCoordinates(Vec2d(phi,theta));
			return true;
		}
	}
	...
	if(y<0.0)
		y*=-1;
	i.setUVCoordinates(Vec2d(0.5-(atan2(P[2],P[0])/(2*3.14)),y));
	//i.setUVCoordinates(Vec2d(double(rand()%100)/100.0,0.5));
	double z = P[2];
	if( z >= 0.0 && z <= 1.0 ) {
		i.t = t2;
